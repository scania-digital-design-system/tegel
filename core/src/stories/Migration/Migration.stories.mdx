...
In order to establish a clear differentiation between them, we have recently renamed the `<ssd-inline-tabs>` as `<sdds-folder-tabs>` to better emphasize both its visual appearance and its functionality.
