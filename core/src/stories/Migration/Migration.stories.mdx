...
// New ✅
<tds-badge value="1" is-visible="true" size="lg"></tds-badge>
```
...
The dropdown has been completely overhauled and redone. This leads to a number of props/event/etc changing names
...
Previously, the data table component provided these props to enable users to implement their own filter, sort, and pagination logic. However, we have now removed these props and replaced them with cancellable events emitted by the component.

The events are: 'tdsPageChange', 'tdsSortChange', 'tdsFilterChange'
...
Previously the table could populate it's rows/columns based on a bodyData property. This has now been removed due to performance reasons.
