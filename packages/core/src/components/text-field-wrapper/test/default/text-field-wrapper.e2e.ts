...
const componentTestPath = 'src/components/text-field-wrapper/test/default/index.html';
const textFieldWrapperSelector = 'tds-text-field-wrapper';
const textFieldInputSelector = 'tds-text-field-wrapper input';

test.describe.parallel('TdsTextFieldWrapper - default state', () => {
  ...
  test('renders default text-field correctly', async ({ page }) => {
    const textField = page.locator(textFieldWrapperSelector);
  ...
  test('should not render label in "no-label" state', async ({ page }) => {
    const textFieldLabelSelector = page.locator('tds-text-field-wrapper label');
    await expect(textFieldLabelSelector).toHaveCount(0);
  });

  test('should not have a prefix', async ({ page }) => {
    const prefixPresence = page.locator('tds-text-field-wrapper span[slot="prefix"]');
    await expect(prefixPresence).toHaveCount(0);
  });

  test('should not have a suffix', async ({ page }) => {
    const suffixPresence = page.locator('tds-text-field-wrapper span[slot="suffix"]');
