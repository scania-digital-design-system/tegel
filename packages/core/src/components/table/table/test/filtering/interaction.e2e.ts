...
testConfigurations.withModeVariantsAndBrands.forEach((config) => {
  test.describe.parallel(getTestDescribeText(config, testDescription), () => {
    ...
    test('look for textbox and click it', async ({ page }) => {
      const tdsTableToolbarSearchButton = page.getByRole('button');
      await tdsTableToolbarSearchButton.click();

      const tdsTableToolbarSearchInput = page.getByRole('textbox');

      await expect(tdsTableToolbarSearchInput).toBeVisible();
      await expect(tdsTableToolbarSearchInput).toBeFocused();
    ...
    test('clicking on search button opens field for entering data', async ({ page }) => {
      const tdsTableToolbarSearchButton = page.getByRole('button');
      await expect(tdsTableToolbarSearchButton).toHaveCount(1);
