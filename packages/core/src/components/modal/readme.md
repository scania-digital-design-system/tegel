...
| `noReferenceElement` | `no-reference-element` | Indicates that there is no reference element used to open the modal, e.g. if it's opened programmatically. Used to disable warning about missing referenceEl or selector prop. | `boolean`                      | `false`     |
