...
- **Commit Messages**: We use [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/). In case you have not used it so far, type `pnpm commit` in your terminal, and an interactive guide will help you formulate the correct message. There is a pre-commit hook that lints the message. We may have strong opinions about commit messages, but they help make our release notes short, informative, and neat.
...
- **Install workspace dependencies**: From the root of the Tegel repository, run `pnpm install`. The pnpm workspace automatically links the local packages together, using the `workspace:` protocol, so no manual linking is required.
- **Build Tegel**: Navigate to your current Tegel monorepo directory, and run `pnpm build:all`. This will build core Tegel as well as all packages currently supported (Angular, Angular 17 & React).
...
- **Building**: Complete the build step for Tegel and verify that the package you plan to test has been built successfully. There are commands available to only build specific packages, i.e., `pnpm build:{package}`.
- **Use the local workspace package**: The wrapper packages automatically reference the local Core package through the pnpm workspace, so no additional setup is required.

### Testing outside the workspace
If you want to test your local Tegel changes in a project that is **not** part of the pnpm workspace, you will need to reference the local package (for example, using `pnpm link` or a local `file:` / `link:` dependency), since the `workspace:` protocol only works within the same pnpm workspace.

## Testing
- **Running Tests**: Run `pnpm test` to run the test suite. It will execute numerous Playwright tests and might take 15-20 seconds to complete all the tests. Tests run on commit at the moment. A successful test will enable committing.
