...
`;
    ...
    const variables = dictionary.allTokens
      .map((token) => {
        ...
        const pathParts = token.path.slice(1).map(
          (part) =>
            part
              .replace(/[🔴🔵❓]/g, '')
              .trim()
...
const themeConfigs = Array.from(brands.values()).reduce((configs, brand) => {
  brand.themes.forEach((theme) => {
    ...
    configs[themeName] = {
      source: ['tokens/json/primitive/default.json', `tokens/json/semantic/${themeName}.json`],
      platforms: {
        scss: {
          transformGroup: 'tokens-studio',
          transforms: ['attribute/cti', 'name/kebab', 'color/css'],
          ...
          files: [
            ...
            },
            {
              ...
              filter: (token) => {
                ...
                if (token.filePath.includes('primitive')) {
                  return (
                    token.original && token.original.$value && token.original.$value.startsWith('{')
                  );
                }
